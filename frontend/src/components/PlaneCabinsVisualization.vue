...
<template>
    <div class="bg-gray-100 rounded-lg max-h-full max-w-full py-6 overflow-auto">
        <div class="h-fit w-fit mx-auto">
            <div v-for="cabin in seatingModel.cabins" :key="cabin.id" class="flex flex-col gap-y-2 mb-6">
                    ...
                    <div class="justify-center flex gap-x-8">
                        <div v-for="isle in cabin.isles" :key="isle.id" class="flex gap-x-2">
                            <div v-for="seat in isle.seats" :key="seat.id">
                                <div class="w-8 md:w-9 lg:w-10 text-center font-medium">{{ seat.colCode }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="w-10"></div>
                ...
                <div v-for="index in cabin.rows" :key="index" class="flex justify-center">
                    <div class="w-10 font-semibold text-md text-gray-600 text-center flex items-center justify-center">{{ getRowIdx(index, cabin.className) }}</div>
